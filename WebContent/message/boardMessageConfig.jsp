...
<html>
<head>
    <TITLE>Board Message Config</TITLE>
...
<style type="text/css">
.pfd_button {
  -webkit-animation: BLINK-PFD-BUTTON 5s infinite; /* Safari 4+ */
  -moz-animation:    BLINK-PFD-BUTTON 5s infinite; /* Fx 5+ */
  -o-animation:      BLINK-PFD-BUTTON 5s infinite; /* Opera 12+ */
  animation:         BLINK-PFD-BUTTON 5s infinite; /* IE 10+, Fx 29+ */
  animation-delay: 0s; 
  animation-iteration-count: infinite;
}

@-webkit-keyframes BLINK-PFD-BUTTON {
  0%   { opacity: 0; }
  10%   { opacity: 1; }
  20%   { opacity: 0; }
  30%   { opacity: 1; }
...
@-moz-keyframes BLINK-PFD-BUTTON {
  0%   { opacity: 0; }
  10%   { opacity: 1; }
  20%   { opacity: 0; }
  30%   { opacity: 1; }
...
$( ".keyb" ).css("display","none");
	...
	$("#send").click(function () {
	  ...
	  $("#sendBoardMessageForm").attr("action","sendBoardMessage").submit();
	});
	
	
	$(".ckmsg").change(function () {
...
<body><div style="background: #111">
   <img src="../images/header2.png" width="100%" height="100px;" alt="header2"/>
<ul>

	<li><a class="tab" style="display: block;color: white;text-align: center;padding: 5px 16px;text-decoration: none;" href="../train/trainRunningInfo">Online Train</a></li>
	<li style=" float: left;"><a  style="display: block;color: white;text-align: center;padding: 5px 16px;text-decoration: none;" <s:if test='%{#session["user"].status==false}'> class="not-active-privilege" </s:if><s:else>class="tab"</s:else> href="../device/deviceStatus">Status</a></li>
	<li style="float: left;"><a  href="../device/deviceConfig" <s:if test='%{#session["user"].display==false}'> class="not-active-privilege" </s:if><s:else>class="tab"</s:else> style="display: block;color: white;text-align: center;padding: 5px 16px;text-decoration: none;">Display</a></li>
	<li style="float: left;"><a  href="../device/deviceIntensity" <s:if test='%{#session["user"].intensity==false}'> class="not-active-privilege" </s:if><s:else>class="tab"</s:else> style="display: block;color: white;text-align: center;padding: 5px 16px;text-decoration: none;">Intensity</a></li>
	<li style="float: left;"><a href="../log-report/logReport" <s:if test='%{#session["user"].logReport==false}'> class="not-active-privilege" </s:if><s:else>class="tab"</s:else> style="display: block;color: white;text-align: center;padding: 5px 16px;text-decoration: none;">Log Report</a></li>
 	<li><a  href="../cctv/led-lcd-display.jsp" <s:if test='%{#session["user"].cctv==false}'> class="not-active-privilege" </s:if><s:else>class="tab"</s:else> style="display: block;color: white;text-align: center;padding: 5px 16px;text-decoration: none;">CCTV</a></li> 
	<li><a  href="../announcement/personalAnc" <s:if test='%{#session["user"].personalAnc==false}'> class="not-active-privilege" </s:if><s:else>class="tab"</s:else> style="display: block;color: white;text-align: center;padding: 5px 16px;text-decoration: none;">PA</a></li>
	                      
	<li class="active" ><a href="getBoardMessageList" style="display: block;color: white;text-align: center;padding: 5px 16px;text-decoration: none;">Message</a></li>
	<li><a  href="../setup/setup.jsp" <s:if test='%{#session["user"].setup==false}'> class="not-active-privilege" </s:if><s:else>class="tab"</s:else> style="display: block;color: white;text-align: center;padding: 5px 16px;text-decoration: none;">Setup</a></li>
	<li><a  href="../option/optionSettings" <s:if test='%{#session["user"].interfaces==false}'> class="not-active-privilege" </s:if><s:else>class="tab"</s:else> style="display: block;color: white;text-align: center;padding: 5px 16px;text-decoration: none;">Interface</a></li>
	<li><a  href="../user/userSetting" <s:if test='%{#session["user"].user==false}'> class="not-active-privilege" </s:if><s:else>class="tab"</s:else> style="display: block;color: white;text-align: center;padding: 5px 16px;text-decoration: none;">User</a></li>
	<li><a class="tab" href="../train/help.jsp" style="display: block;color: white;text-align: center;padding: 5px 16px;text-decoration: none;">Help</a></li>
	<li style="float: right"><a class="tab" href="../logout" style="display: block;color: white;text-align: center;padding: 5px 16px;text-decoration: none;">Logout</a></li>
	<li style="float: right"><a class="tab" href="../user/changePassword" style="display: block;color: white;text-align: center;padding: 5px 16px;text-decoration: none;"><s:property value="#session['user'].userId" /></a></li>
...
<div id="displayneww">
<center>
<form action="addBoardMessage" method="post"><br>
<table>
<tr><td>
<label>Select Language </label>&nbsp;&nbsp;
</td><td>
<s:if test="%{id!=0}">
<input type="hidden" name="boardMessage.id" value="<s:property value='boardMessage.id'/>">
</s:if>
<s:iterator  value="languageList" status="list">
<input type="radio" id="<s:property value="languageName"/>" <s:if test="languageName == 'English'"> checked="checked" </s:if>  class="language" name="language" style="width: 20px; height: 20px;"><label><s:property value="languageName"/></label>&nbsp;&nbsp;
</s:iterator>
<script type="text/javascript">
var id='<s:property value='boardMessage.id'/>';
if(id!=0)
	{
	   var lang='<s:property value='boardMessage.boardMessage'/>';
	   var radioButton=document.getElementsByName("language");
	   if(lang.charCodeAt(0)<=255)
		   {
		   document.getElementById(radioButton[0].id).checked=true;
		   }
	   else if(lang.charCodeAt(0)>=2304 && lang.charCodeAt(0)<=2431)	
		   {
		   document.getElementById(radioButton[1].id).checked=true;
		   }
	   else if(lang.charCodeAt(0)>=2432 && lang.charCodeAt(0)<=2559)	
	   {
	   document.getElementById(radioButton[2].id).checked=true;
	   }
 ...
 <!-- <input type="radio"   id="hin" class="language" name="language"  style="width: 20px; height: 20px; "><label>Hindi</label> -->
</td></tr>
<tr><td>&nbsp;</td></tr>
<tr><td>
<label>Board Message</label>
</td><td>
<textarea  id="keyboard" style="width: 400px;height:50px;" name="boardMessage.boardMessage" required><s:if test="%{id!=0}"><s:property value='boardMessage.boardMessage'/></s:if></textarea></td></tr>

</table>
<br>
<div id="buttons" style="text-align: center">
<button class="btn orange tab" id="add">Save Message</button></div>
...
<div  style="width: 60%;  margin: 0 auto; background: rgba(0, 0, 0, 0.90);">
<form method="post" id="sendBoardMessageForm">
<div style="height: 300px;overflow: scroll;">
<table class="table1" style="background: white;">
<thead>
<tr>
...
<s:set var="size" value="boardMessageList.size" />
<s:iterator value="boardMessageList" status="list">
<tr>
<td><input type="checkbox" <s:if test="%{sendMessage==true}">checked</s:if> style="width: 20px;height: 20px" class="ckmsg" name="messageId" value='<s:property value="id"/>'></td>
<td><s:property value="boardMessage"/></td>
<td><a class="tab" href="getBoardMessageDetail?id=<s:property value="id"/>">
<img alt="" src="../images/edit.png" width="25px" height="25px"></a></td>
<td><a class="tab" href="deleteBoardMessage?id=<s:property value="id"/>">
<img alt="" src="../images/Delete_Icon.png" width="25px" height="25px"></a></td>

<s:if test="%{#list.index==0}">
<td class="tab"><font color="#428bca">UP</font></td>
</s:if>
<s:else>
<td><a class="tab" href="swapBoardMessageRow?id1=<s:property value="id"/>&id2=<s:property value="boardMessageList[#list.index-1].id"/>">UP</a></td>
</s:else>

<s:if test="%{#list.index==#size-1}">
<td class="tab"><font color="#428bca">Down</font></td>
</s:if>
<s:else>
<td><a class="tab" href="swapBoardMessageRow?id1=<s:property value="id"/>&id2=<s:property value="boardMessageList[#list.index+1].id"/>">Down</a></td>
</s:else>
...
<a class="btn orange tab" id="send" style="background:transparent;padding: 0px 0px;"><img id="pfd_button" src="../images/platform.png" width="150" height="80"/></a></div>
<div id="processing" style="float:right; top:780px;right:40px; left:850px; display:inline-block; ; vertical-align: top; margin-top: 30px; display: none;">
